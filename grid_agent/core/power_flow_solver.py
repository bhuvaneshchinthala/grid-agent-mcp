...
# =============================================================================
# POWER FLOW SOLVER WRAPPER (for pandapower networks)
# =============================================================================

class PowerFlowSolver:
    """
    Wrapper class for power flow analysis using pandapower.
    Provides run() and detect_violations() methods compatible with the app.
    """
    
    def __init__(self):
        if not PANDAPOWER_AVAILABLE:
            raise ImportError("pandapower is required for PowerFlowSolver")
    ...
    def run(self, net):
        ...
        Returns:
            net: Updated network with power flow results
        """
        try:
            pp.runpp(net, enforce_q_lims=True, calculate_voltage_angles=True)
            self.last_result = "converged"
        except Exception as e:
            # Try with different settings
            try:
                pp.runpp(net, algorithm='nr', init='flat')
                self.last_result = "converged_alternative"
            except Exception as e2:
                self.last_result = f"failed: {e2}"
                raise e2
        return net
    
    def detect_violations(self, net, v_min=0.95, v_max=1.05, loading_max=100.0):
        """
        Detect voltage and thermal violations in the network.
        
        Args:
            net: pandapower network (after power flow)
            v_min: Minimum acceptable voltage (p.u.)
            v_max: Maximum acceptable voltage (p.u.)
            loading_max: Maximum acceptable line loading (%)
            
        Returns:
            dict: Dictionary containing violation details
        """
        violations = {
            "voltage": [],
            "thermal": [],
        ...
        # Check if power flow results exist
            ...
            for line_idx in net.res_line.index:
                ...
                if loading > loading_max:
                    violations["thermal"].append({
                        ...
                        "overload_percent": float(loading - loading_max)
                    })
        
        # Calculate summary statistics
        total_violations = len(violations["voltage"]) + len(violations["thermal"])
        total_elements = len(net.bus) + len(net.line)
        health = max(0, 100 - (total_violations / max(1, total_elements) * 100))
        
        # Severity score based on violation magnitude
        severity = 0
        for v in violations["voltage"]:
            severity += v["deviation"] * 50
        for t in violations["thermal"]:
            severity += t["overload_percent"] * 0.5
        
        violations["summary"] = {
            "total_violations": total_violations,
            "voltage_violations": len(violations["voltage"]),
            "thermal_violations": len(violations["thermal"]),
